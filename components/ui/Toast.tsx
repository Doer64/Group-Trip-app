...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const toastContainer = (
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-2.5 pointer-events-none max-w-sm w-full px-4">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto flex items-center justify-between p-3.5 rounded-2xl shadow-2xl border text-sm backdrop-blur-xl animate-in fade-in-0 slide-in-from-bottom-4 duration-300 ease-out ${
            t.type === 'success'
