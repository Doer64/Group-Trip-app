...
export default function InvitePage() {
  ...
  const loginRedirect = `/login?redirect=${encodeURIComponent(`/invite/${token}`)}&message=${encodeURIComponent(`You were invited to a trip to ${tripData.destination}! Sign in so your friends know who you are.`)}`;

  return (
    <div className="py-8 flex items-center justify-center min-h-[58vh]">
      <div className="bg-white/95 rounded-[2rem] border border-slate-200 p-6 sm:p-8 shadow-xl shadow-slate-900/5 max-w-md w-full mx-auto text-center relative overflow-hidden">
        <div className="mb-6">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 border border-amber-200/60 text-amber-600 flex items-center justify-center mx-auto mb-3 shadow-2xs">
          ...
          <div className="mt-4 bg-slate-50 rounded-2xl p-4 border border-slate-200/80">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Organized by <span className="font-bold text-slate-800">{tripData.creatorName}</span>
            </p>
          </div>
        </div>
